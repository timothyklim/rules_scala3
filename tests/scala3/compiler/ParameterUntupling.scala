...
object ParameterUntupling:
  ...
  def test: Unit =
    ...
    /** Current behaviour in Scala 2.12.2 : error: missing parameter type Note: The expected type requires a one-argument function accepting a
      * 2-Tuple. Consider a pattern matching anonymous function, `{ case (s, i) => ... }`
      */
