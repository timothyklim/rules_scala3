...
def _worker_scala_binary_impl(ctx):
...
worker_scala_binary = rule(
    ...
    doc = "Simplifyed `scala_binary` to create workers and avoid recursion.",
    executable = True,
    outputs = {
        "bin": "%{name}-bin",
        "jar": "%{name}.jar",
    ...
    implementation = _worker_scala_binary_impl,
    toolchains = [
